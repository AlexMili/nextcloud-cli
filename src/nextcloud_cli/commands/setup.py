...
def login(url: str | None, username: str | None, password: str | None, timezone: str | None) -> None:
    """Log in to a Nextcloud server.

    Values are taken from CLI flags first, then environment variables
    (``NEXTCLOUD_URL`` / ``NEXTCLOUD_USER`` / ``NEXTCLOUD_TOKEN`` /
    ``NEXTCLOUD_TIMEZONE``), and only what is still missing is prompted for.
    """
    ...
    emit({"status": "logged-in", "url": url, "username": username, "timezone": timezone})
